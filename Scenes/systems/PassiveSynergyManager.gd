...
signal tower_bonuses_updated(tower: Turret, bonuses: Dictionary)
...
func get_towers_in_range(center_position: Vector2, range: float) -> Array[Turret]:
	var towers_in_range: Array[Turret] = []
...
func get_adjacent_towers(tower: Turret) -> Array[Turret]:
	var adjacent_towers: Array[Turret] = []
	var all_towers = get_all_deployed_towers()
...
func get_all_deployed_towers() -> Array[Turret]:
...
func calculate_capture_tower_synergy(tower: Turret) -> Dictionary:
...
func calculate_attack_speed_aura(tower: Turret) -> Dictionary:
...
func calculate_mage_damage_synergy(tower: Turret) -> Dictionary:
...
func calculate_stealth_detection_bonus(tower: Turret) -> Dictionary:
...
func calculate_unique_damage_bonus(tower: Turret) -> Dictionary:
...
func calculate_adjacent_da_ta_boost(tower: Turret) -> Dictionary:
...
func calculate_slowed_enemy_bonus(tower: Turret) -> Dictionary:
...
func update_all_tower_bonuses():
	...
	for tower in all_towers:
		if is_instance_valid(tower):
			...
			tower.passive_da_bonus = bonuses.get("da_bonus", 0.0)
			tower.passive_ta_bonus = bonuses.get("ta_bonus", 0.0)
			tower.passive_damage_bonus = bonuses.get("damage_bonus", 0.0)
			tower.passive_speed_bonus = bonuses.get("speed_bonus", 0.0)
			
			tower_bonuses_updated.emit(tower, bonuses)

## Apply effects from other towers to this tower
func apply_cross_tower_effects(tower: Turret, base_bonuses: Dictionary, all_towers: Array[Turret]) -> Dictionary:
